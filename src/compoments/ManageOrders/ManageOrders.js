...
const ManageOrders = () => {
    ...
    const handleDelete = (id) => {
        ...
        if (confirmation) {
            fetch(`https://fierce-sierra-20822.herokuapp.com/allorders/${id}`, {
                method: 'DELETE',
    ...
    const handleUpdatestatus = (id) => {
        ...
        const url = `https://fierce-sierra-20822.herokuapp.com/update/${id}`;
        fetch(url, {
            method: 'PUT',
            headers: {
    ...
    useEffect(() => {
        fetch('https://fierce-sierra-20822.herokuapp.com/allorder')
            .then(res => res.json())
                ...
                <div className="order">
                    {
                        allorder.map(order => <div key={order._id} className="col-md-4 g-2 mt-4 od">
                            ...
                            <button className="btn btn-primary" onClick={() => handleUpdatestatus(order._id)}>{order?.status}</button>
